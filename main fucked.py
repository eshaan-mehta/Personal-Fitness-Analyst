...
def reset() -> None:
    ...
    global straightness_warnings, foot_warnings, frames, depth_points, rep_points, frame_count
    global anim_count_back, anim_count_knee, graph
...
#-----------------------------------------------------------------------------
    ...
    if status == 3:
        ...
        frame = cv.putText(img=frame,
                        text=f"Total Foot Warnings: {foot_warnings}",
...
def graph_overlay(frame: cv.Mat) -> cv.Mat:
    w, h = frame.shape[1]//2, frame.shape[0]//2
    x_off, y_off = frame.shape[1]//4, frame.shape[0]//4 
...
#-------------- FRONT/BACK KEYPOINT DETECTION ----------------------
            ...
            if status == 2:
                ...
                if front_knee_angle < FRONT_FOOT_THRESHOLD and not is_up and anim_count_knee == 0:
                    foot_warnings += 1
                    anim_count_knee = 1
            ...
            elif keypoints[10][1] < keypoints[6][1]: #set status to "end" if right wrist above shoulder
                status = 3
            else: 
